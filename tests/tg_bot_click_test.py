...
def test_cli_import_session_copies_into_selected_session(tmp_path):
    ...
    result = runner.invoke(tg_bot_click.app, ["--import-session", str(source), "--session", str(target)])
...
def test_cli_export_session_copies_from_selected_session(tmp_path):
    ...
    result = runner.invoke(tg_bot_click.app, ["--session", str(source), "--export-session", str(target)])
...
def test_cli_rejects_missing_credentials_before_importing_telethon():
    result = runner.invoke(tg_bot_click.app, ["@example_bot", "--match", "签到"])
...
def test_cli_help_documents_session_migration():
    result = runner.invoke(tg_bot_click.app, ["--help"])
...
def test_root_command_registers_tg_bot_click():
    ...
    result = runner.invoke(root_cmd, ["tg-bot-click", "--help"])
